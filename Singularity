...
%runscript
    ...
    /planner/fast-downward.py \
        ...
        $PROBLEMFILE \
         ...
         --search "astar(lmcut, pruning=stubborn_sets_simple(minimum_pruning_ratio=0.01), num_por_probes=1000)"

## Update the following fields with meta data about your submission.
## Please use the same field names and use only one line for each value.
%labels
Name        Metis1
Description Remake of Metis that participated in IPC2014. Uses partial order reduction and symmetry pruning (OSS).
